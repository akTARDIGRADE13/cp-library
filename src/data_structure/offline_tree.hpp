...
struct OfflineTree {
    ...
    int idx_of(const T& x) const {
        return static_cast<int>(std::distance(xs.begin(), std::ranges::lower_bound(xs, x)));
    }

    bool valid_index(int i, const T& x) const { return i < n && xs[i] == x; }

    int count(const T& x) const {
        int i = idx_of(x);
        if (!valid_index(i, x)) return 0;
        return ft.sum(i, i + 1);
    }
    ...
    bool insert(const T& x) {
        int i = idx_of(x);
        if (!valid_index(i, x)) return false;
    ...
    int order_of_key(const T& x) const {
        int r = static_cast<int>(std::distance(xs.begin(), std::ranges::lower_bound(xs, x)));
        return ft.sum(0, r);
    }
    ...
    int count_le(const T& x) const {
        int r = static_cast<int>(std::distance(xs.begin(), std::ranges::upper_bound(xs, x)));
        return ft.sum(0, r);
    }
    ...
    std::optional<T> lower_bound(const T& x) const {
        int pos = static_cast<int>(std::distance(xs.begin(), std::ranges::lower_bound(xs, x)));
    ...
    std::optional<T> upper_bound(const T& x) const {
        int pos = static_cast<int>(std::distance(xs.begin(), std::ranges::upper_bound(xs, x)));
